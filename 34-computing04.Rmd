...
This exercise is modelled on one JH used in the [607 course in 2001,](http://www.medicine.mcgill.ca/epidemiology/hanley/c607/ch06/homegrown_exercises_06.pdf#page=3) but now using data from all [US births in 2018](https://www.cdc.gov/nchs/data_access/vitalstatsonline.htm)

 The birthweights within a panel pertain to infants of the same sex, but different panels may pertain to different sexes.
...
Whereas, as an individual 'investigator', you might be luck or unlucky in which of the 4 comparisons you get right, the real issue is the **probabilities of getting them right**. That is why in 2001 JH had each student in the large 707 class , and why he tabulated their results, so we got a better sense of waht these probabilities are. And better still, in 2020, you alone can use the computer to get an even better sense.
...
When you review each test result, you can only classify them by column, by 'engatibe' or 'positive'. And in real life, that would be as far as you can go, since your are not privy to the 'truth'. But, here, we have the _luxury of knowing_ which comparisons are of each type, so we can croos-classify the results, and examine the avearge performance of these tests. In this way, we get a feel for the probabilities that they will perfrtom they way we  want them to, and can see how the probabilities are modified by the amount of effort (sample size) we are willing to employ, as well as how small/large a signal, and how small/large the noise, we are up against.

So, for now, in your one set of 4 hand-worked examples, you can put the 4 test results into two columns. Below, when we will do  a lar ge number of tests, we will reveal which comparisons are of which type, so you can doubly-classift them.

**Exercises**

1.  For each of the 4 contrasts listed above, use a 'reproducible' random selection method to choose 4 infants from each of the two specified panels. [Note: since each entry represents almost 200,000 infects, the sampling from the 100 values (quantiles) should be with replacement. This may not matter a lot when you are sampling just 4 infants, but if you were sampling 16 or 64 or 256, it would matter] Compute the difference between the sample meansNote which contrasts yielded 'statistically significant' differences (i.e. differences of 6xx: these can be the first entries in your 2 x 2 table below, once the 'identities'truth' is revealed.


**Cross-classification of results of statistical tests [columns] in relation to real situations[rows]**
