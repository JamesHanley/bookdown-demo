...
1. Using the year-of-manufacture frequency distribution you derived back then, canculate the mean year, and thus the mean age.
...
In the resulting [data file](http://www.biostat.mcgill.ca/hanley/statbook/LengthsMostPopularCars.txt), each of the 448 rows 
...
1. Read the data into an `R dataframe` using
...
ds=read.table("http://www.biostat.mcgill.ca/hanley/statbook/LengthsMostPopularCars.txt",as.is=TRUE)
str(ds)
...
3. What if he had subtracted them and calculated the variance of the difefrences?
...
This exercise is modeleled on one JH used in the [607 course in 2001,](http://www.medicine.mcgill.ca/epidemiology/hanley/c607/ch06/homegrown_exercises_06.pdf#page=3) but now using data from all [US births in 2018](https://www.cdc.gov/nchs/data_access/vitalstatsonline.htm)

 The birthweights with a panel pertain to infants of the same sex, but different panels may pertain to different sexes.
...
They were asked to use new (fresh) samples of size $n$ = 4 and $n$ = 4 for each of the 4 tests.

These sample sizes will (rightly) strike you as being way to small to reliably distinguish two populations whose means are not that far apart. Indeed, [most of the students in 2001 were unable to correctly identify the 
opposite-sex panels](http://www.medicine.mcgill.ca/epidemiology/hanley/c607/ch06/telling_sexes_apart.pdf#page=1) 

But, when it comes to distinguishing same-sex panels from   opposite-sex panels of **adult heights**, most of the students in 2001 **were able** to [correctly identify the opposite-sex panels](http://www.medicine.mcgill.ca/epidemiology/hanley/c607/ch06/telling_sexes_apart.pdf#page=2), with only a few mistaking the same-sex panels for opposite-sex ones.   

Although it makes no sense, they -- like so many textbooks start with --  were told that the Population Standard Deviation ($\sigma$) was known. The main reason for being allowed this 'inside knowledge' (typically reserved for Gods and Oracles) 
was that it made the computation of the (z-) test statistic easier:  students only had compute the two sample means, easily done by hand, and avoided having to compute sample standard deviations. For now, we will do the same, but later will also go on and carry out  real-world tests that don't rely on 'inside' information about standars deviations. [After all, if you know the Population Standard Deviation, then you must already know the Population Mean.]  
...
For interest, here is what the critical distance is for other sample sizes: $n_1 = n_2 = 16:$ 341 grams; \ $n_1 = n_2 = 64:$ 171 grams; \ $n_1 = n_2 = 256:$ 85 grams.
...
#    227    2920    3240    3203    3560    8165 
#ds$Sex: M
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#    227    3015    3364    3317    3690    8165 
# aggregate(ds$BirthWeight,by=list(ds$Sex),sd)
#       F 573.5224
#       M 600.9757
# 

Q=matrix(c(
 862,1540,1871,2060,2185,2285,2355,2420,2475,2523,
2567,2608,2637,2665,2693,2722,2750,2775,2792,2810,
2835,2855,2867,2891,2905,2920,2940,2950,2970,2980,
3000,3010,3030,3035,3055,3062,3080,3090,3105,3118,
3130,3145,3150,3170,3175,3190,3203,3210,3230,3232,
3250,3260,3270,3289,3290,3310,3317,3330,3345,3353,
3370,3375,3395,3402,3417,3430,3440,3459,3465,3485,
3490,3510,3520,3540,3550,3570,3580,3600,3610,3629,
3650,3660,3685,3701,3720,3742,3770,3790,3810,3836,
3865,3895,3930,3969,4010,4060,4120,4200,4320,4564,
 878,1560,1905,2102,2240,2350,2430,2495,2552,2608,
 2645,2690,2722,2750,2780,2810,2835,2863,2890,2910,
 2930,2948,2974,2990,3005,3030,3040,3060,3070,3090,
 3104,3119,3135,3147,3160,3175,3190,3204,3216,3232,
 3240,3260,3265,3285,3290,3310,3317,3330,3345,3355,
 3371,3378,3395,3402,3420,3430,3440,3459,3462,3480,
 3489,3506,3515,3530,3544,3558,3572,3580,3600,3610,
 3629,3640,3657,3670,3685,3700,3714,3732,3744,3770,
 3780,3799,3820,3840,3856,3884,3905,3930,3950,3980,
 4008,4040,4080,4111,4160,4210,4280,4360,4479,4725),
100,2)
...
#points(c(0.25,0.25,0.75,0.75),
#       c(0.25,0.75,0.25,0.75),pch=19)
dx=0.043; dy =dx
...
for( i.r in 1:2){
  yc = 0.25 + (i.r==2)*0.5
  for(i.c in 1:2){
    if(i.r==i.c)  w = sample(Q[,1]) + (i.r==1) - (i.r==2)
    if(i.r!=i.c)  w = sample(Q[,2]) + (i.r==1) - (i.r==2)
    xc = 0.25 + (i.c==2)*0.5
...
To help with rapid compilation of results in class, circle below which contrasts yielded "statistically significant" differences and BRING YOUR 8 DECISIONS TO CLASS.


 Birthweights
Adult Heights
"Arithmetic" of Testing if 2 panels have same mean
Bvs.C B vs. C


2. ...

On class, I will 'play god' and tell you which contrasts belong in which rows. In practice, you may not be able to unequivocally determine the truth -- or it may take a lot more work. And determining how big a difference is takes even more work.
Results of statistical tests [columns] performed by students in relation to real situations[rows]
