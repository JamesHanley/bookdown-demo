...
This one is less artificial, and indeed is motivated by a real court case in the alte 1990s in Quebec, where a defendant's age (which would determine wheter he was tried in an adult or a juvenile court) was in doubt. He was adopted, while still a young child, from another country. Official birth records were not available, and his adoptive parents were able to get a cheaper airfare by claiming that he was under age 2 at the time. Bone age, and Tanner Staging, also known as Sexual Maturity Rating (SMR), an objective classification system used to track the development and sequence of secondary sex characteristics of children during puberty, were other pieces of information used by the judge.
...
Consider first a single indirect measurement of chronological age, that yielded a value of **17.1 years**.

Given what you know about the sizes of the possible errors, you **cannot say** that the true age is **exactly** 17.1 years What **can** you say? And with what certainty?

You can put **limits** on the true age by asking **what are the minimum and maximum ages that could have produced the observed reading of 17.1 years**.
...
draw = function(conf.level){
  ...
  for (example in 1:2) {
     ...
     plot(c(0,a.max),c(0,y.max),col="white",
      ...
      polygon(c(0,a.max,a.max,0),c(0,y.max,y.min,0),col="grey90",border=NA)
      ...
      i.age = 17.3
      if(example>1) i.age = c(17.3, 16.1, 19.9)
      n=length(i.age)
      U = a.max ; L = 0
      for (i in 1:n){
       ...
       segments(i.age[i]/(1+radius.as.proportion*q.u),
                ...
                0-i*dy,lwd=1.25,
                col=c("black","red")[1+(conf.level<1)])
       points(i.age[i],0-i*dy, pch=19,cex=0.75)
