...
for (example in 1:2) {
   ...
   plot(c(0,a.max),c(0,y.max),col="white",
       xlab="True Chronological Age",ylab="Indirect Measure of Age",
       xlim=c(0,1.1*a.max),
   ...
   i.age = 13.1
   if(example>1) i.age = c(13.1, 15.5, 11.2)
   n=length(i.age)
   U = a.max ; L = 0
   for (i in 1:n){
     points(0, i.age[i], pch=19)
     if(example==1) text(0,i.age[i],
            toString(i.age[i]), cex=0.75, adj=c(0.5,-0.75))
     segments(0,i.age[i],a.max,i.age[i],lty="dotted")
     for(rr in 1+c(-1,0,1)*radius.as.proportion ){
       if(example==1) arrows(i.age[i]/rr, i.age,
            i.age[i]/rr, 0.5, length=0.07,angle=30)
     }
     segments(i.age[i]/(1+radius.as.proportion),
              0-i*dy,
            i.age[i]/(1-radius.as.proportion),
              0-i*dy,lwd=1.25)
     U = min(U,i.age[i]/(1-radius.as.proportion))
     L = max(L,i.age[i]/(1+radius.as.proportion))
   } # i
