...
Moreover, there is **no doubt that the greatest deviation has its limits which are never exceeded** and which indeed are narrowed by the experience and skill of the observer. Beyond these limits all probability is zero; from the limits towards the mark in the centre the probability increases and will be greatest at the mark itself.
...
We must wonder whether its was that this eminent mathematician (who also, incidentally contributed to the epidemiological debate about smallpox vaccination) was unable to think of a formula for an error distribution that did not end abruptly, and that would instead 'flow out' further in both directions. Or was it that he wasn't bothered have required having to find the roots of 5th degree polynomials to find the best 'centre' of his semicircukar distribution? As it turns out, the mathematics involved in findling (what we now call today) the Maximun Likelihood Estimator for Laplace's and Gauus's (infinitely wide) error distributions is much simpler. But the, we have the befit of hiundsight. 
...
q = function(p){
 cdf = 
   function(y) 1/2 + (y*sqrt(1-y^2))/pi + asin(y)/pi - p 
 return(uniroot(cdf, c(-1,1))$root)
}


draw = function(conf.level){
  q.u = q( (1+conf.level)/2 )
  a.max=30; da=2.5
  beta = 1
...
We will cut 5% from each corner. That means a 90% probability of a measurement falling between
`r paste(-round(q.u,2))` $\times$ the radius of the semi-ellipse and 
`r paste("+",round(q.u,2),sep="")` $\times$ the  radius, leaving  only a 5% probability of observing a measurement below this, and 
a 5% probability of observing a measurement above this. After we observe a single measurement, we retain only those true-age-scenarios in which the observed measurement would fall within this central (90%) range, and discard those age scenarios in which  the measurement would be at one extreme or the other extreme, ie, in which  the measurement would be in the excluded or 'cut' corner.

The left panel shows the (now narrower, and more nuanced) true-age-interval that is compatible with the observed measurement of 13.1 years. In all other age-scenarios, the 13.1 would have been too extreme, and so these scenarios are discarded.  

Note again the method of constructing this non-symmetric interval, namely one boundary at a time. It does not fit  the $\pm$ mold.

It does, however, give a way of talk about such an interval:

the observed measurement (point-estimate) may be an underestimate: it might have fallen short of the true value. Or,  it may be an underestimate: it might have overshot the true value. The plus and the minus amounts are the almost-maximal amounts by which it might have done so. (as we will see later, the maximal error can be infinite, so we have to put some probalistic limits on the error if we are to narrow the interval).
