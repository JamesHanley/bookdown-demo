...
In the past, when introducing simple linear regression, it was common to learn the formula by heart, and use it compute by hand, the standard error for a fitted slope, namely
$$SE[ \hat{\beta} ] = \sqrt{\frac{\sum (y-\bar{y})(x-\bar{x})}{ \sum (x-\bar{x})^2}}.$$
Sadly, [this formidable formula hides what is going on](http://www.medicine.mcgill.ca/epidemiology/hanley/Reprints/SimpleMultipleLinearRegressionSampleSize.pdf). 

To truly understand what is going on, lets consider a very simple example. A student from a country that uses the Fahrenheit (F) system moves to Montreal, and wishes to know how to translate the outside temperature, expressed as the number of degrees Celsius (C) and shown in the Metro, and heard on radio stations, back into the F scale (s)he is familiar with. The student knows that the conversion is of the form  F = $\alpha$ + $\beta \times C$ but, rather than look them up on Google, decides to estimate $\alpha$ and $\beta$ from pairs of (C,F) readings, taking the C readings directly from the Metro screen, and the F ones from his/her own portable thermometer. Suppose that the C readings are displayed to 1 decimal place, but that the F thermometer only displays the F to the nearest integer (or more generally, has a random error of -0.5F in half the occasions, and +0.5F in the other half).
...
Given that these two C settings correspond to 54.5 F and 63.5 F,  what are the possible $\beta$ estimates? And, how variable are they?

The 4 possibilities, shown below, are (64-55)/5, (64-54)/5, (63-55)/5, and (63-54)/5, or, when sorted, 1.6, 1.8, 1.8 and 2.0, each with probability 1/4, so that the variance is
$$Var[ \hat{\beta} ] =  \frac{(1.6 - 1.8)^2 + (1.8 - 1.8)^2  + (1.8 - 1.8)^2 + (2.0 - 1.8)^2}{4} = 0.02,$$
...
$$Var[random  \ slope] = (1/5)^2 \times ( Var[e_1] + Var[e_2]).$$ 

```{r,eval=T, echo=F, fig.align="center", fig.height=5, fig.width=9, warning=FALSE, message=F, fig.cap="Estimating the using s."}
...
points(x[1],Y[1],cex=0.5,pch=19)
arrows(x[1],Y[1]-1/2, x[1],Y[1]+1/2,
       length=0.06, angle=45,code=3,lwd=1)
text(x[1]-2/5,Y[1],
    bquote(e[1]),  adj=c(0.5,1), srt=90,
    cex=1)
text(x[1]-2/5,Y[1]+5,
  bquote("Var[" ~ e[1] ~ "] = " ~ frac(-0.5^2+0.5^2)(2) ),
  adj=c(0,1), cex=1)
  
```
For heuristic reasons, consider first the simplest possible case of just (n =) 2 X locations, Y errors that could equally be +1 or -1 (so that se2 = 1), and estimating a slope (whose true value is B = 4, say) by dividing the observed difference in the 2 y’s by the difference in their X locations. We use ‘y’ to denote an ‘error-containing’ Y.
If the two Xs are 1 unit apart, then with errors of +/-1 the resulting values of y will differ by one of (4 - 1 - 1) = 2, (4 - 1 + 1) = 4, (4 +1 - 1) = 4, and (4 + 1 + 1) = 6, each with equal probability. So, the slope estimate is equally likely to be 2, 4, 4, or 6; thus, the error in this estimate is equally likelytobe-2,0,0,or2;andsothesamplingvariancewillbe50%of22 +50%of02 =2.If instead, the Xs are two units apart (as they are in the ‘LONG1’ design), so that 2B = 8, the four equally likely slope estimates will now be 6/2, 8/2, 8/2, and 10/2. Thus, the errors in these estimates are equally likely to be -1, 0, 0, and 1, and so their variance will be 50% of (1)2 + 50% of 02 = 1/ 2, four times smaller than if the readings were only 1 X unit apart.
