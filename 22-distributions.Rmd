...
The important point of this simple regression example is that even though in practice there would be many more data points, the principle/law used to calculate the sampling variation of a slope based on any number of datapoints remains the same: the slope is still (in this case) an $x$-based linear combination of $y$'s, and thus an $x$-based linear combination of random errors -- or more broadly of random deviations from the $x$-conditional means of the '$Y$' variables. We will return later to all of the factors that influence the narrowness/width of  sampling distributions generally, but you can maybe already see from the 'algebraicly isolated' representation of the slope that the more datapoints -- and the wider apart they are on the $x$ axis and the smaller the magnitudes of the errors --- the more reproducible the slope. The influence of this latter factors is less evident in the textbook formula for the Variance and the SE. This little exercise (next) should help you fiogure out how the factors come into it. JH also covers these is [this piece].  

**Exercise**: (i) What if the student took the F readings at two C values that are 10 C (rather than 5 C) apart?, i.e. at 12.5 C and 22.5 C? How would the Variance and the SE be altered?  (ii) What if the student took the F readings at four equally spaced C values 5 C apart, i.e., at 7.5 C, 12.5 C, 17.5C and 22.5 C?. If you were limited to $n$ C values, how would you decide where to place them? (iii) What if, rather than $0.5^2$, the 'errors' in F had a variance of $1^2$ or $2^2$?  
