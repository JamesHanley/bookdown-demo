...
The foolowing tow distributions are taken from 'Student's 1907 article 
...
for(panel in 1:2){
   ...
   for (i.r in 1:20){
      for(i.c in 1:20){
         ...
         y =y.m [i.r,i.c]
        #text(i.c-1/2, 21-i.r-1/2, toString(y),
        #     col="grey25", cex=0.5 )
...
```{r,eval=T, echo=F, fig.align="center", fig.height=3, fig.width=9, warning=FALSE, message=F,fig.cap="Examples of distributions of events in space -- or in population time if you think of the horizontal axis as the passage of time, and the verytical axis as a fixed-size (but dynamic) population, or as several (say age-) strata with fixed sizes."}

par(mfrow=c(1,1),mar = c(0,0,1,0))

w=20;h=10;

plot(0:w, 0:w/2, "s",col = "white",
    xlab="time",
    ylab="population")

for (i in 0:w)  abline(v= i,   col ="lightgray", lty="dashed")
for (j in 0:h)  abline(h= j,   col ="lightgray", lty="dashed")

for (i in seq(0,w,2))  abline(v= i,   col ="lightgray", lty="solid")
for (j in seq(0,h,2))  abline(h= j,   col ="lightgray", lty="solid")


x=c(); y=c();
for (i in 1:w) {
  for (j in 1:h) {
  n=rpois(1,2*exp(0.015*2*floor((i+1)/2)+0.030*2*floor((j+1)/2)))
  if (n>0) { x=c(x, (i-1) + runif(n) ) ; y=c(y, (j-1) + runif(n) ) }
}
}
...
w=20 ; d.w = 0.001; n.trials = w/d.w; lambda = 0.7; p = lambda*d.w ;

x=c(); y=c();

for (strip in 1: strips) {
  for (i in 1:n.trials) {
    if (runif(1) < p) { x=c(x, i*d.w ) ; y=c(y, strip/strips) }
  }
}
...
plot(0:w, (0:w)/w, "s",col = "white",
    xlab="time",
    ylab=" ", frame.plot=F )

for (strip in 1: strips) abline(h= strip/strips,   col ="lightgray")
for (i in 0:w)  abline(v= i,   col ="lightgray")
...
Events in Population-Time randomly generated from intensities that are constant within (2 squares high by 2 squares wide) `panels', but vary between such panels. In Epidemiology, each square might represent a number of units of population-time, and each dot an event.}

Events in Time: 10 examples [1/row], randomly generated from constant over time intensities. Simulated with 1000 Bernoulli$(\tiny{\textrm{small }\pi})$'s per time unit.}

**Does the Poisson Distribution apply to$\dots$?**
