...
for (n in sample.sizes ){
   ages = matrix(sample(AGES,
                    size = n*no.of.sims, 
                    replace = TRUE,
                    prob = Proportions),
                 nrow = no.of.sims, ncol=n)
   print(head(ages,3))

   sums.samples.of.size.n = apply(ages,MARGIN=1,FUN=sum)
   means.samples.of.size.n = apply(ages,MARGIN=1,FUN=mean)

   tail(sums.samples.of.size.n,4)
   tail(means.samples.of.size.n,4)

   fr = table(sums.samples.of.size.n)
   Y = max(Proportions*no.of.sims)/sqrt(0.8*n)
   ...
   for(a in seq(0,100,5)) {
     ...
     if(a==100) text(n*105, -0.01*Y,
          paste("Sum of",toString(n),"Ages"),adj=c(0,1),cex=1.5)
     text(n*a, -0.15*Y, toString(a),adj=c(0.5,1),font=4,cex=1.5)
     if(a==100) text(n*105, -0.15*Y,
