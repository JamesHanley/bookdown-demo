...
5. Set up a long vector to contain the sample means of many simulated random **pairs** of measurements of (possibly imperfect copies) of the meter bar. For example, you might specify `no.of.pairs = 10000` and then `means.of.samples.of.size.2 = rep(NA,no.of.pairs)`. This would set up a vector of length 10000, with all entries inially set to `NA`.

Using the 2-point (-1cm, +1cm) error distribution shown in the first Figure, simulate a large number (enough so that the shapes of the histograms are reproducible) of possible random **pairs** of measurements (possibly imperfect) of the meter bar. For each of these simulated samples of size $n$ = 2, calculate the sample mean, and insert it into the location if  
