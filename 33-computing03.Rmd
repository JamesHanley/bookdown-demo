...
The **'computing' objective** is to learn how to use `R` to evaluate probabilities by (Monte Carlo) simulation. `R` Functions/structures used include `sample`, `matrix`, `apply`, `paste`, `cumprod` (cumulative product), `duplicated`, `unique` 
...
first.duplicate = function(x) min( (1:n.hermits)[duplicated(x)] )
...
[mini-version of 'the birthday problem'] 
