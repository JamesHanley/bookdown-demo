...
Across the performances **up to mid April**,  the (rounded) variance of `r format(round(v.tot,4), nsmall = 4)` is  **a lot more than** the variances in their **'real'** (longer-term, remaining) performances. Much of the initial variation, about `r format(round(v.binomial,4),nsmall = 4)` worth, is nothing more than binomial variation that you would see with $n$'s  of the order of 60, and  player-specific true proportions that vary from maybe 0.225 to 0.325. [for example, at $\pi$ =0.3 and $n$ = 60, the binomial variance is 0.3 * 0.7 / 60 = `r format(round(0.3*0.7/60,6),nsmall=4)`. Thus, only about `r format(round(v.net,4),nsmall=4)` of it (about `r round(100*v.net/v.tot)`%) reflects the variance of the player-specific true (long-term) success rates.
...
Clearly,  most of the many of individual binomial-based point estimates and many of the individual binomial-based frequentist confidence intervals (which we will consider in the next section) that do not use the knowledge that individual season-long averages are confined to a narror band between say 0.250 and 0.325 will not come close to or 'trap' the eventual individual longer-term batting averages. The correct way to make better frequentist intervals is to consider the family of players as an ensemble. If we label a randomly selected player as '_rs_' say, then the long-run average $\pi_{rs}$  is thought of (modelled) as drawn from the distribution (family) of long-run averages. Then  _rs_'s short-term average can be though of as  drawn from (as a realization of) a binomial distribution with parameter $\pi_{rs}$ and sample size $n_{rs}$.



```{r,eval=T, echo=F, fig.align="center", fig.height=8, fig.width=9, warning=FALSE, message=F, fig.cap="Batting averages of selected Major League baseball players, 2019"}
...
for(i in 1:length(P)) { 	 
	fr = d[i]
	segments(P[i],0,  P[i], fr, col=hc[i], lwd=5,lend=2,lend=2 )
	extra = dbinom(x,n,P[i]) * fr * 1.85 ; 
	#print(round(max(extra),4))
	d1 = d0 + extra
	#print( round(cbind(d0,extra),2 ) )
	for(j in 1:(n+1)) rect(x[j]/n-dx, d0[j], 
	                       x[j]/n+dx, d1[j], 
	                     col=hc[i], border=NA)
    d0 = d1 
}
