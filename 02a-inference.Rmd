...
```{r,eval=T, echo=F, fig.align="center", fig.height=9, fig.width=9, warning=FALSE, message=F, fig.cap="If If he looked like he was around 60."}
...
expression(paste("Parameter of Interest: ",
                 theta,
                 " = A Specific Person's True log[Cholesterol Level]")), cex=1,adj=c(0,0))

segments(0,2.7, 3,2.5,lty="dotted",lwd=2)

segments(v0,0,v1,0,lwd=0.5,col="lightblue")
text(1,1.75,"Distrn.: True log[Level]s in all such persons", cex=0.6,adj=c(0,0))
text(1,0.7,"[ Vertical axis such that a.u.c. = 1 ]", cex=0.5,adj=c(0,0))
...
for (v in seq(v0,v1,dv)) {
	segments(v,-0.1,v,0)
	text(v,-0.15,toString(v),cex=0.5,adj=c(0.5,1))
	} 
...
for (v in seq(v0,v1,dv)) {
	...
	text(v,y0-0.2,toString(v),cex=0.5,adj=c(0.5,1))
	} 
text(observed.log.chol,y0-1,"y",cex=0.6,adj=c(0.5,0.5),font=3)
text(1,y0-1,"Observed log[level] for person in question",  
   cex=0.6,adj=c(0,0.5))
points(c(observed.log.chol),y0,pch=19,cex=0.5)
text(v1+0.5*dv,y0,
 expression(
   paste(
     italic(Y)
...
text(v1+0.5*dv,y0,
 ...
 text(1, y0-2,"Adapted from Irwig et al, JAMA 1991",cex=0.6,adj=c(0,0),
 font=3)
